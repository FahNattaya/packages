...
test.describe("AIS Mobile Shop Device Sale Check Contract SCN 001 Check contract กด checkbox และ กดปุ่ม Next", async () => {
  test("ผู้ใช้งาน ยอมรับสัญญาในหน้า contract กด checkbox และ กดปุ่ม Next", async ({ page }) => {
    ...
    await test.step("ผู้ใช้งานตรวจสอบหน้า contract เลือก ข้าพเจ้ายอมรับตามเงื่อนไขข้อความข้างต้น กดปุ่ม NEXT", async () => {
      await navBar.navbarDisplay();
      await customerNavBar.customerNavbarDisplay();
