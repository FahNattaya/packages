...
test.describe("AIS Mobile Shop Device Sale Check Contract SCN 002 User กดปุ่ม back ที่หน้า contract กลับไปหน้าถ่ายรูป", async () => {
  test("ผู้ใช้งาน กดปุ่ม back ที่หน้า contract กลับไปหน้าถ่ายรูป", async ({ page }) => {
    ...
    await test.step("Device Sale Redesign SP3_AIS Mobile Shop_Generate Contract_Product_SCN 01", async () => {
    ...
    await test.step("ผู้ใช้งานตรวจสอบ ตระกร้าสินค้า กดปุ่ม PAY NOW", async () => {
      ...
      await test.step("ผู้ใช้งานตรวจสอบหน้า User ไม่ยอมรับสัญญา ไม่ยอมกด checkbox ปุ่ม next disable ", async () => {
        await navBar.navbarDisplay();
        await customerNavBar.customerNavbarDisplay();

        await contract.clickBack();
      });
  });
});
