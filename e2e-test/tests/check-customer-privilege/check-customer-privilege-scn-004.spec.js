...
test.describe('AIS Shop Device Sale Customer Privilege SCN 004 Check Prepaid Fail', async () => {
    test('ระบบตรวจสอบสิทธิ์ของลูกค้าเบอร์ Prepaid ไม่ผ่าน', async ({ page }) => {
        ...
        await test.step('ผู้ใช้งานกรอกเบอร์ลูกค้าที่ต้องการทำรายการ', async () => {
            await validateCustomer.inputMobileNumber('0937050288');
        ...
        await test.step('ผู้ใช้งานเลือกเบอร์โทรศัทพ์ของลูกค้า', async () => {
            ...
            await listNumber.pillChargeTypeDisplay('Prepaid');
        ...
        await test.step('ผู้ใช้งานเลือก Campaign และ Trade', async () => {
            ...
            await productSelling.dialogUnableToAddToCart('INVALID_STAY_NTYPE');
