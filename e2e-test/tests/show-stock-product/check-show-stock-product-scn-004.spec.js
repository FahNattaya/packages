...
test.describe("AIS Mobile Shop Device Sale Check Show Stock Product SCN 004 ระบบแสดงสีหมด stock onhand แต่มี online ตัวหนังสือสีแดง", async () => {
  test("ระบบแสดงสีหมด stock onhand แต่มี online ตัวหนังสือสีแดง", async ({ page }) => {
    ...
    await test.step("ผู้ใช้งานเลือกสินค้าที่ต้องการซื้อ", async () => {
      ...
      await handsetList.selectBrandName('iPhone 12 Pro 256GB');
    ...
    await test.step("ผู้ใช้งานตรวจสอบ product-selling เลือก Campaign และ Trade แล้ว กดปุ่ม ADD TO CART", async () => {
      ...
      await handsetList.pillCategoryIsSelected('handset');
      await productSelling.checkDisabled("GOLD");
      await productSelling.checkColorDisplay("GOLD");
      await productSelling.checkStockColorDisplay("GOLD","0");
      await productSelling.checkTextStockOnline("GOLD")

      await productSelling.checkDisabled("GRAPHITE");
      await productSelling.checkColorDisplay("GRAPHITE");
      await productSelling.checkStockColorDisplay("GRAPHITE","0");
      await productSelling.checkTextStockOnline("GRAPHITE")

      await productSelling.checkDisabled("PACIFIC BLUE");
      await productSelling.checkColorDisplay("PACIFIC BLUE");
      await productSelling.checkStockColorDisplay("PACIFIC BLUE","0");
      await productSelling.checkTextStockOnline("PACIFIC BLUE")
